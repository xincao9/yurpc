...
public class TimerDAO {
    ...
    private static final String TIMER_BY_METHOD = "select method, ct, sum(one_minute_rate) as one_minute_rate, sum(five_minute_rate) as five_minute_rate, sum(fifteen_minute_rate) as fifteen_minute_rate from timer where method=:method group by ct order by ct limit 100";
    ...
    public List<Timer> getTimerByMethod(String method) {
        ...
        return namedParameterJdbcDaoSupport.getNamedParameterJdbcTemplate().query(TIMER_BY_METHOD, sqlParameterSource, (rs, rowNum) -> {
